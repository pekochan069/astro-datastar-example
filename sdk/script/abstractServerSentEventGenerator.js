...
class ServerSentEventGenerator {
    ...
    send(event, dataLines, options) {
        ...
        const retryLine = [
            `retry: ${retryDuration ?? consts_js_2.DefaultSseRetryDurationMs}\n`,
        ];
    ...
    hasDefaultValue(key, val) {
        if (key === consts_js_1.DefaultExecuteScriptAttributes.split(" ")[0]) {
            return val === consts_js_1.DefaultExecuteScriptAttributes.split(" ")[1];
        }
        if (key in types_js_1.DefaultMapping) {
            return val === types_js_1.DefaultMapping[key];
        }
        return false;
    }
    /**
